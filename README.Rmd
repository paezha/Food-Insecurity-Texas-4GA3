...
This paper reports an analysis of food insecurity in Texas, USA, and its relationship with race. The analysis was carried out at the tract level. Data were obtained from the US Census.
...
library(stargazer) # Well-Formatted Regression and Summary Statistics Tables # package for creating tables with output of models 
...
library(textreg) # n-Gram Text Regression, aka Concise Comparative Summarization
...
```{r, include=FALSE}
# Check notebook 00-Retrieve-Data.Rmd in folder data for the process of creating these data objects
load(glue::glue(here::here(), "/data/df_race.rda"))
load(glue::glue(here::here(), "/data/df_race_c.rda"))
```
...
To confirm that we’ve successfully removed residual spatial autocorrelation in the SEM specification, a Moran’s I test on the residuals was employed.The result of Moran’s I = –0.341 (p = 1), which does not allow us to reject the null hypothesis of independence. This suggests that the SEM specification has successfully absorbed spatial dependence in the residuals, thus validating the reliability of our regression coefficients. These results are illustrated below, with the Morans I test and a map of the spatially independent residuals. 
...
# Lagged residuals
l_res_sem <- lag.listw(df_race_c.w, df_race_c$r_sem)

# Test
moran_result <- moran.test(l_res_sem, df_race_c.w, alternative = "less")
```

```{r, echo=FALSE}
ggplot(df_race_c) +
  geom_sf(aes(fill = l_res_sem > 0), color = "white", size = 0.2) +
  scale_fill_manual(
